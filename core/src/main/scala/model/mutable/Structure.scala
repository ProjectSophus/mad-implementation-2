...
object Structure {
    case class Concept (relatedObjects : collection.mutable.Buffer[String] = collection.mutable.Buffer()) extends Structure {
        def toStructure = immutable.Structure.Concept(relatedObjects.toSeq)
    }
